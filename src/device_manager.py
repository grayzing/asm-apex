...
class DeviceManager:
    def __init__(self, num_devices: int) -> None:
        self.device_position_matrix: np.ndarray = np.zeros((num_devices, 3), dtype=np.float16)
        self.device_velocity_matrix: np.ndarray = np.zeros((num_devices, 3), dtype=np.float16)
