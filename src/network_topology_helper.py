...
class HexagonalNetworkTopologyHelperWithRandomDevicePlacements(NetworkTopologyHelper):
    ...
    def generate_base_station_positions(self):
        ...
        for i in range(self.base_station_manager.base_station_position_matrix.shape[0]):
            ...
            base_station_positions.append([x_offset, y_offset, 10.0]) 
...
class HeterogenousHexagonalNetworkTopologyHelperWithRandomDevicePlacements(HexagonalNetworkTopologyHelperWithRandomDevicePlacements):
    def __init__(self, num_base_stations: int, num_sectors_per_base_station: int, num_devices: int, inter_site_distance_m: float = 500.0, seed: int = 24) -> None:
        ...
        self.sector_manager.tx_power_dbm_matrix = np.full((self.sector_manager.num_sectors, ), 35.0, dtype=np.float32) # This is the Tx power in dBm over the entire bandwidth.
        self.sector_manager.bandwidth_mhz_matrix = np.full((self.sector_manager.num_sectors, ), 100.0, dtype=np.float32) # This is the bandwidth in MHz.
        self.sector_manager.sector_numerology_matrix = np.full((self.sector_manager.num_sectors, ), 3, dtype=np.int8) # This is the numerology index.

    def generate_base_station_positions(self):
        # Generate hexagonal grid positions for base stations
        super().generate_base_station_positions()
        # Assign center frequencies based on the reuse pattern
        ...
        self.sector_manager.center_freq_ghz_matrix = np.repeat(base_station_center_freqs, self.sector_manager.num_sectors // self.base_station_manager.base_station_position_matrix.shape[0])

        # Change first base station to UMa cell
        self.sector_manager.center_freq_ghz_matrix[0:3] = [6.0, 6.5, 7.0]  # UMa cell frequencies
        ...
        self.base_station_manager.base_station_position_matrix[0] = 25.0
