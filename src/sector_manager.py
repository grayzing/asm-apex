...
class SectorManager:
    def __init__(self, num_sectors: int, num_base_stations: int):
        ...
        self.center_freq_ghz_matrix: np.ndarray = np.full((num_sectors, ), 6.0, dtype=np.float16)
        self.bandwidth_mhz_matrix: np.ndarray = np.full((num_sectors, ), 20.0, dtype=np.float16)
        self.tx_power_dbm_matrix: np.ndarray = np.full((num_sectors, ), 49.0, dtype=np.float16) # This is the Tx power in dBm over the entire bandwidth.
        self.sector_numerology_matrix: np.ndarray = np.full((num_sectors, ), 1, dtype=np.int8)
        self.sector_physical_resource_block_utilization: np.ndarray = np.full((num_sectors, ), 0.05, dtype=np.float16)
        self.device_bits_per_prb: int = 1000
