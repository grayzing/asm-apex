...
class RadioUnitPowerConsumptionHelper:
    """
    Implements a heterogeneous O-RU power consumption model where multiple sectors
    can map to a single physical O-RU hardware unit (e.g., a 3-sector 64T64R Macro).
    """
    ...
    def calculate_digital_components_power(self, sleep_mode_manager: SleepModeManager) -> np.ndarray:
        """
        Calculates the digital component power. For the shared macro O-RU, the digital power 
        overhead is driven by the shallowest sleep mode among its active sectors.
        """
    ...
    def calculate_rf_processing_power(self, sleep_mode_manager: SleepModeManager) -> np.ndarray:
        """
        Calculates total RF power per sector. RF power is modeled per sector antenna array.
        """
        sleep_modes = sleep_mode_manager.sector_sleep_mode_matrix
        clamped_modes = np.clip(sleep_modes, 0, 4)
    ...
    def calculate_analog_components_power(self, sleep_mode_manager: SleepModeManager) -> np.ndarray:
        """
        Calculates analog component power (25% of RF power per sector).
        """
    ...
    def calculate_power_amplifier_power(self, sleep_mode_manager: SleepModeManager) -> np.ndarray:
        """
        Calculates Power Amplifier power (75% of RF power per sector).
        """
    ...
    def calculate_total_power(self, sleep_mode_manager: SleepModeManager) -> np.float32:
        """
        Computes absolute power consumption summing all components correctly aligned to hardware mapping.
        """
