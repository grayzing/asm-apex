...
def test_update_downlink_received_signal_power_dbm_matrix_um_a():
    ...
    sector_mgr.center_freq_ghz_matrix = np.array([2.0], dtype=np.float32)
    ...
    assert model.downlink_received_signal_power_dbm_matrix.shape == (1, 1)
    assert model.los_boolean_matrix.shape == (1, 1)
    assert model.shadowing_db_matrix.shape == (1, 1)
    assert model.path_loss_db_matrix.shape == (1, 1)
    assert model.breakpoint_distance_matrix.shape == (1, 1)
