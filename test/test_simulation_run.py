...
def test_simulation_logging_run():
    num_gnbs = 1
    num_devices = 3
    num_sectors_per_gnb = 3

    sector_mgr = SectorManager(num_sectors=num_gnbs * num_sectors_per_gnb, num_base_stations=num_gnbs)
    traffic_model = ParetoDistributionTrafficModel(time_frame_ms=5, num_devices=num_devices, seed=123)
    ...
    assert len(metrics["sinr"]) == 3
    assert len(metrics["total_throughput"]) == 3
    assert len(metrics["average_throughput"]) == 3
    assert len(metrics["p10_throughput"]) == 3
    assert all(value >= 0.0 for value in metrics["total_throughput"])
    assert all(value >= 0.0 for value in metrics["average_throughput"])
    assert all(value >= 0.0 for value in metrics["p10_throughput"])
